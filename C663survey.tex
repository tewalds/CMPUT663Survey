...
System event notifications are perhaps one of the simplest forms of an event system application. Well known examples include the Microsoft COM+ event architecture and the Mac OS X NSNotificationCenter architecture. 
...
While the sender sends a message to a specific email address, that address may be associated with many recipients. Some addresses are forwarded to many recipients, which can form the basis of a mailing list. Other addresses are forwarded to many devices, such as a desktop, laptop, and mobile phone. While an email address is a specific address, it is actually closer to a subscription to a fairly broad set of messages in the publish/subscribe model. If the subscription wasn't so broad, spam wouldn't be such a big problem.
...
If you want a timely response from a specific other party, event-based middleware is totally inappropriate as the foundation of your communications.
