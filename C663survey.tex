...
Keeping these challenges into mind, the paper has implemented a four layer middleware system. The first layer is the objects layer where objects in the game encapsulate state of the game and manipulate the states in a consistent manner with the help of some actions. The second layer is the duplicate objects layer where the state of the games that needs to be distributed to players is encapsulated by duplicated objects. The third layer is the communication abstraction layer where the duplicated objects middleware layer communicates with other nodes. There are two subsections in this method. The first one is remote method invocation. When a duplicated object performs an operation, this operation is called to maintain consistency issues. The calls of methods are synchronous. They have exactly one semantics. The second subsection of communication abstraction layer is publish/subscribe. It updates the states of all duplicas after the state of a duplication master has changed. Here several subscriber nodes at a time are capable of expressing interest on a particular channel. This layer of middleware is implemented in a way such that it can handle the cases of inconsistency which occur when a node having duplica is modified. Thus this layer plays a very important role in the overall architecture. The final layer is the network layer which lies in the lowest level provides the functionality of messaging. The paper provides different implementations of the middleware and different network layer experiments which reflect the importance of object oriented middleware in the world of multiplayer online games.
